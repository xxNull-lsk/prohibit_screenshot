...
int isProhibitScreenshot(uint32_t* lock_image_id);
...
Image* loadImage(const char* filename) {
	...
	if (!fp) {
		fprintf(stderr, "load lock image failed!filename=%s\n", filename);
		return NULL;
	}
...
int isProhibitScreenshot(uint32_t* lock_image_id)
{
    ...
    shm_data = (LOCK_IMAGES*)mmap(NULL, sizeof(LOCK_IMAGES), PROT_READ, MAP_SHARED, shm_fd, 0);
