...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        printf("用法: %s <PNG文件> <IMAGE文件>\n", argv[0]);
        return 1;
    }
    Image* image = load_png(argv[1]);
    if (!image) {
        return 1;
    }
    ...
    // 将ARGB转换成RGB
    unsigned char* data = (unsigned char*)malloc(image->width * image->height * 3);
    memset(data, 0, image->width * image->height * 3);
    for (int i = 0; i < image->width * image->height; i++) {
        data[i * 3] = image->data[i * 4 + 0];
        data[i * 3 + 1] = image->data[i * 4 + 1];
        data[i * 3 + 2] = image->data[i * 4 + 2];
    }
    fwrite(&image->width, 4, 1, fp);
    fwrite(&image->height, 4, 1, fp);
    fwrite(data, 1, image->width * image->height * 3, fp);
    fclose(fp);
